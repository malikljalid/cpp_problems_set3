...
void calculateSumOfColsOfMatrix(int matrix[ROWS][COLS], int SumOfRow[ROWS], short int Rows, short int Cols)
{
    for (short int i = 0; i < Rows; i++)
    {
        for (short int j = 0; j < Cols; j++)
            SumOfRow[j] += matrix[i][j];
    }
}

void printMatrixSumOfCols(int SumOfMatrixRows[ROWS], int Rows)
{
    std::cout << "\nThe following is the sum of each row in the matrix : \n";

    for (int i = 0; i < Rows; i++)
        std::cout << "Col " << i << " Sum = " << SumOfMatrixRows[i] << std::endl;
}
...
int main(void)
{
    ...
    calculateSumOfColsOfMatrix(matrix, SumOfMatrixRows, ROWS, COLS);
    printMatrixSumOfCols(SumOfMatrixRows, ROWS);
