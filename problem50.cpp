...
void deleteClient(std::string fileName, std::string delim, std::string toDelete)
{
    std::vector <stBankRecord> vNewRecords = LoadDataFromFileToVectorExceptFor(fileName, delim, toDelete);
...
int main(void)
{
    ...
    else
    {
        ...
        std::cout << "Are you sure you want to delete this client (Yes-y / No-n) ? : ";
        std::cin >> choice;

        if (choice == 'y')
        {
            deleteClient("clients_bankRecords.txt", ":", toFindAccountNumber);
            std::cout << "Client Deleted Successfuly!\n";
        }
    }
